...
function formatExperience(value) {
  const months = Number(String(value ?? "").match(/\d+(?:\.\d+)?/)?.[0]);
  if (!Number.isFinite(months) || months < 0) return value;
  ...
  return remainingMonths ? `${years} ${years === 1 ? "year" : "years"} ${remainingMonths} ${remainingMonths === 1 ? "month" : "months"}` : `${years} ${years === 1 ? "year" : "years"}`;
}
