...
const DEFAULT_MAP_LINK = "https://www.google.com/maps?q=Swargate%2C%20Pune&output=embed";
...
export default function ContactUs() {
  ...
  return <div><PageHeader title="Contact Us" crumb="Contact Us" /><section className="bg-cream py-8 md:py-12"><div className="container-app">
    ...
    <div className="grid items-stretch gap-6 lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <div className="overflow-hidden bg-navy shadow-[0_8px_30px_rgba(11,37,69,0.08)]"><div className="flex items-center justify-between border-b border-white/10 p-5 text-white"><div><p className="text-xs font-bold uppercase tracking-wider text-gold">Find us</p><h2 className="mt-1 text-xl font-bold">Swargate, Pune</h2></div><MapPin className="text-gold" /></div><iframe title="Shri Shahu Prabodhini location in Swargate Pune" src={getMapEmbedUrl(contactInfo?.mapLink)} className="h-[360px] w-full border-0 md:h-[420px]" loading="eager" allowFullScreen referrerPolicy="no-referrer-when-downgrade" /><a href={contactInfo?.mapLink || "https://www.google.com/maps/search/?api=1&query=Swargate%2C%20Pune"} target="_blank" rel="noreferrer" className="flex items-center justify-center border-t border-white/10 bg-navy px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#263238] hover:text-gold">Open in Google Maps <MapPin size={16} className="ml-2 text-gold" /></a></div>
    </div>
  </div></section></div>;
}
